...
impl Guess {
    pub fn new(value: i32) -> Guess {
        if value < 1 {
            panic!("Guess value must be greater that or equal to 1, got value {value}");
        } else if value > 100 {
...
mod tests {
    ...
    // in the body of tests, which can be a convenient way to write tests that should fail
    ...
    fn it_works_with_results_instead_of_panicking() -> Result<(), String> {
        if 2 + 2 == 4 {
            ...
            Err(String::from("2 + 2 doesn't equal to 4"))
        }
    }
}
